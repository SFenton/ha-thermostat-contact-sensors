...
# =============================================================================
# Tests for Additional Coverage - Integration Style
# =============================================================================


class TestFanModesIntegration:
    """Integration tests for fan mode functionality."""

    def test_fan_modes_with_thermostat_that_supports_fan(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test fan mode selection when thermostat supports fan control."""
        from unittest.mock import MagicMock
        
        # Create thermostat state with fan modes
        mock_state = MagicMock()
        mock_state.attributes = {
            "fan_modes": ["auto", "on", "high", "low"],
            "fan_mode": "auto",
        }
        mock_hass.states.get.return_value = mock_state
        ...
        # Should get available fan modes
        available = controller.get_available_fan_modes()
        assert "auto" in available
        assert "on" in available

    def test_fan_modes_with_thermostat_without_fan_support(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test graceful handling when thermostat doesn't support fan."""
        from unittest.mock import MagicMock
        
        # Create thermostat state without fan modes
        mock_state = MagicMock()
        mock_state.attributes = {}  # No fan_modes attribute
        mock_hass.states.get.return_value = mock_state
...
class TestTargetTemperatures:
    """Tests for target temperature handling."""

    def test_get_target_temperatures_from_global_thermostat(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test getting target temps from global thermostat."""
        from unittest.mock import MagicMock
        ...
        # Mock global thermostat
        mock_global = MagicMock()
        mock_global.target_temperature_low = 20.0
        mock_global.target_temperature_high = 24.0
        mock_global.hvac_mode = HVACMode.HEAT_COOL
        controller._global_thermostat = mock_global

        low, high, single = controller.get_target_temperatures()
        assert low == 20.0
        assert high == 24.0

    def test_get_target_temperatures_heat_mode_only(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test target temps in heat-only mode."""
        from unittest.mock import MagicMock
        ...
        mock_global = MagicMock()
        mock_global.target_temperature_low = 21.0
        mock_global.target_temperature_high = None
        mock_global.hvac_mode = HVACMode.HEAT
        controller._global_thermostat = mock_global
...
class TestRoomTemperatureState:
    """Tests for RoomTemperatureState utility class."""

    def test_room_temperature_state_with_no_readings(self):
        """Test RoomTemperatureState when no temperature readings available."""
        ...
        assert room_state.available_sensor_count == 0
        assert room_state.has_valid_readings is False

    def test_room_temperature_state_single_sensor(self):
        """Test RoomTemperatureState with single sensor."""
        room_state = RoomTemperatureState(
            area_id="bedroom",
            area_name="Bedroom",
        )
        room_state.sensor_readings = {"sensor.temp": 21.5}

        assert room_state.available_sensor_count == 1
        assert room_state.has_valid_readings is True

    def test_room_temperature_state_multiple_sensors(self):
        """Test RoomTemperatureState with multiple sensors."""
        room_state = RoomTemperatureState(
            area_id="bedroom",
            area_name="Bedroom",
        )
        ...
        assert room_state.available_sensor_count == 3
        # Test get_closest_to_target instead of non-existent methods
        sensor, temp = room_state.get_closest_to_target(22.0, HVACMode.HEAT)
        assert temp is not None


class TestCycleProtection:
    """Tests for cycle protection features."""

    def test_cycle_protection_prevents_rapid_cycling(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test that cycle protection prevents rapid on/off cycling."""
        controller = ThermostatController(
            hass=mock_hass,
            thermostat_entity_id=TEST_THERMOSTAT,
            occupancy_tracker=mock_occupancy_tracker,
        ...
        # Just turned on 2 minutes ago
        recent_time = now - timedelta(minutes=2)
        controller.update_cycle_time(HVACMode.HEAT, recent_time)

        # Should NOT be able to turn off yet (within 5 min window)
        assert controller.can_turn_off(now) is False

    def test_cycle_protection_allows_turn_off_after_minimum(
        self, mock_hass, mock_occupancy_tracker
    ):
        """Test cycle protection allows turn off after minimum time."""
        controller = ThermostatController(
            hass=mock_hass,
            thermostat_entity_id=TEST_THERMOSTAT,
            occupancy_tracker=mock_occupancy_tracker,
        ...
        # Turned on 6 minutes ago
        long_ago = now - timedelta(minutes=6)
        controller.update_cycle_time(HVACMode.HEAT, long_ago)

        # Should be able to turn off now
        assert controller.can_turn_off(now) is True


# TestFanModeSelection and TestComplexRoomScenarios classes removed
# These tests were testing private methods and didn't follow proper test patterns
# The functionality they were testing is already covered by integration tests
