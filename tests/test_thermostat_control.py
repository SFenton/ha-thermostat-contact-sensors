...
class TestEcoMode:
    ...
    When eco mode is enabled, the thermostat should prioritize active (occupied)
    rooms for *normal* control decisions. Critical temperature protection should
    still be evaluated and can trigger HVAC operation.
    """
    ...
    def test_eco_mode_does_not_suppress_critical_room(self, controller, mock_hass):
        """Test that eco mode does not suppress critical unoccupied rooms."""
        inactive_area = AreaOccupancyState(
            area_id=TEST_AREA_BEDROOM,
            area_name="Bedroom",
            binary_sensors=[],
        ...
        # With eco mode, critical room should still be counted for control
        ...
        assert state_eco.critical_room_count == 1
    ...
    def test_eco_mode_no_action_when_no_active_rooms(self, controller, mock_hass):
        ...
        def get_state(entity_id):
            if entity_id == TEST_THERMOSTAT:
                ...
                mock_state = MagicMock()
                mock_state.state = "16.0"  # Critical - should still be protected
                return mock_state
            return None
        ...
        # Critical protection should still be active under eco mode
        assert state.critical_room_count == 1
        assert state.recommended_action != ThermostatAction.TURN_OFF
    ...
    def test_eco_mode_with_active_and_inactive_rooms(self, controller, mock_hass):
        ...
        With eco mode: should still stay on (critical room protection)
        ...
        # With eco mode - critical room is still protected
        state_eco = controller.evaluate_thermostat_action(
            active_areas=[active_area],
            area_temp_sensors=area_temp_sensors,
            inactive_areas=[inactive_area],
        ...
        assert state_eco.recommended_action == ThermostatAction.NONE
        assert "critical" in state_eco.action_reason.lower()
    ...
    def test_eco_mode_respects_anomaly_detection(self, controller, mock_hass):
        ...
        def get_state(entity_id):
            if entity_id == TEST_THERMOSTAT:
                ...
                mock_state = MagicMock()
                # Cold enough to bias house trend towards HEAT, but not cold enough
                # to be in the unoccupied *critical* range (critical protection overrides eco mode).
                mock_state.state = "67.0"
                return mock_state
            return None
...
class TestEcoAwayTargets:
    """Tests for eco_away_targets functionality.
    
    When eco_away_targets is provided, the thermostat control should use these
    targets instead of the individual area targets.
    """
    ...
    def test_eco_away_targets_override_area_targets(self, controller, mock_hass):
        """Test that eco_away_targets overrides individual area targets."""
        inactive_area = AreaOccupancyState(
            area_id=TEST_AREA_BEDROOM,
            area_name="Bedroom",
            binary_sensors=[],
        ...
        def get_state(entity_id):
            if entity_id == TEST_THERMOSTAT:
                mock_state = MagicMock()
                mock_state.state = HVACMode.HEAT
                mock_state.attributes = {"temperature": 72.0}  # Normal target: 72
                return mock_state
            elif entity_id == TEST_TEMP_SENSOR_1:
                mock_state = MagicMock()
                mock_state.state = "64.0"  # Below normal target but above eco away target
                return mock_state
            return None

        mock_hass.states.get.side_effect = get_state

        # Without eco_away_targets - room at 64°F is below 72°F target (needs heat)
        state_normal = controller.evaluate_thermostat_action(
            active_areas=[],
            area_temp_sensors=area_temp_sensors,
            inactive_areas=[inactive_area],
            eco_mode=False,
            eco_away_targets=None,
        )
        # With normal targets, 64°F vs 72°F target = 8°F difference, should be critical
        # (unoccupied_heating_threshold is 3.0)
        assert state_normal.critical_room_count == 1

        # With eco_away_targets (62°F, 85°F) - room at 64°F is ABOVE 62°F target
        # So it's no longer critical for heating
        state_eco_away = controller.evaluate_thermostat_action(
            active_areas=[],
            area_temp_sensors=area_temp_sensors,
            inactive_areas=[inactive_area],
            eco_mode=False,  # eco_mode=False but eco_away_targets provided
            eco_away_targets=(62.0, 85.0),
        )
        # With eco away targets, 64°F is above 62°F target, so not critical
        assert state_eco_away.critical_room_count == 0

    def test_eco_away_targets_applied_to_all_areas(self, controller, mock_hass):
        """Test that eco_away_targets applies to all areas consistently."""
        inactive_area_1 = AreaOccupancyState(
            area_id=TEST_AREA_BEDROOM,
            area_name="Bedroom",
            binary_sensors=[],
            sensors=[],
        )
        inactive_area_2 = AreaOccupancyState(
            area_id=TEST_AREA_LIVING_ROOM,
            area_name="Living Room",
            binary_sensors=[],
            sensors=[],
        ...
        def get_state(entity_id):
            if entity_id == TEST_THERMOSTAT:
                mock_state = MagicMock()
                mock_state.state = HVACMode.HEAT
                mock_state.attributes = {"temperature": 72.0}
                return mock_state
            elif entity_id == TEST_TEMP_SENSOR_1:
                mock_state = MagicMock()
                mock_state.state = "63.0"  # Just above eco away heat target
                return mock_state
            elif entity_id == "sensor.living_temp":
                mock_state = MagicMock()
                mock_state.state = "61.0"  # Just below eco away heat target - critical
                return mock_state
            return None

        mock_hass.states.get.side_effect = get_state

        # With eco_away_targets (62°F, 85°F):
        # - Bedroom at 63°F > 62°F target = NOT critical
        # - Living room at 61°F < 62°F target = potentially critical
        state = controller.evaluate_thermostat_action(
            active_areas=[],
            area_temp_sensors=area_temp_sensors,
            inactive_areas=[inactive_area_1, inactive_area_2],
            eco_mode=False,
            eco_away_targets=(62.0, 85.0),
        )

        # Bedroom should not be critical (63°F > 62°F)
        assert TEST_AREA_BEDROOM in state.room_states
        # The bedroom should not be critical since it's above the eco away heat target
        assert state.room_states[TEST_AREA_BEDROOM].is_critical is False

        # Living room at 61°F with 62°F target and 3°F threshold:
...
class TestTrackedAreaIds:
    ...
    def test_tracked_area_ids_empty_tracks_no_rooms_for_decisions(self, controller, mock_hass):
        """Test that an empty tracked_area_ids set means track NONE for decisions."""
        mock_hass.states.get.side_effect = lambda entity_id: {
            TEST_THERMOSTAT: self._create_thermostat_state(mock_hass, HVACMode.HEAT),
            "sensor.bedroom_temp": self._create_temp_sensor_state(55.0),  # Critical but not tracked
        }.get(entity_id)
        ...
        # Bedroom is still evaluated for display
        assert result.room_states["bedroom"].is_critical is True
        # But no rooms are tracked for decisions
        assert result.critical_room_count == 0
        assert result.active_room_count == 0

    def test_force_critical_area_ids_allows_untracked_critical_in_eco_mode(self, controller, mock_hass):
        """Test that force_critical_area_ids can trigger critical protection under eco mode."""
        mock_hass.states.get.side_effect = lambda entity_id: {
            TEST_THERMOSTAT: self._create_thermostat_state(mock_hass, HVACMode.HEAT),
            "sensor.bedroom_temp": self._create_temp_sensor_state(55.0),  # Critical
        }.get(entity_id)
        ...
        result = controller.evaluate_thermostat_action(
            ...
            eco_mode=True,
            tracked_area_ids=set(),
            force_critical_area_ids={"bedroom"},
        )
